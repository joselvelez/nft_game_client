...
import { fetchDefaultCharacters, fetchUsedCharacterIndexArray, mintNewCharacter } from "../contracts/contractAPI";
import loadingIndicator from '../images/loadingIndicator.gif';
import MintNotificationSuccess from "./MintNotificationSuccess";
  
export default function MintNewCharacterSelection({ setCurrentComponent }) {
  ...
  const [usedCharacterIndexArray, setUsedCharacterIndexArray] = useState([]);

  useEffect(() => {
    console.log("Loading default characters")
    loadUsedCharacterIndexArray();
    loadCurrentTeam();
  }, []);

  async function loadCurrentTeam() {
    try {
      const _team = await fetchUsedCharacterIndexArray();
      if (_team.length > 0) {
        setUsedCharacterIndexArray(_team);
      }
    } catch (e) {
      console.log("Unable to fetch current team", e);
    }
  }

  async function loadUsedCharacterIndexArray() {
    try {
                ...
                <div className="space-y-1">
                  <div className="flex flex-row">
                    ...
                    <div className="mx-auto my-auto p-2">
                      <button
                        type="button"
                        disabled={usedCharacterIndexArray.includes(parseInt(person.characterIndex)) ? true : false}
                        ...
                        className="inline-flex items-center px-3.5 py-2.5 border border-transparent text-xs font-medium rounded shadow-sm text-white
                          bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-30"
                      >
                        Mint
                      </button>
                    </div>
